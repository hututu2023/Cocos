      ...
      _export("GameManager", GameManager = (_dec = ccclass('GameManager'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Vec3), _dec5 = property(Vec3), _dec6 = property(Vec3), _dec(_class = (_class2 = class GameManager extends Component {
        ...
        initBottomCards() {
          ...
          this.bottomCardRoot.children.slice(0, 3).forEach((node, index) => {
            var cardView = node.getComponent(_crd && CardView === void 0 ? (_reportPossibleCrUseOfCardView({
              error: Error()
            }), CardView) : CardView);
        ...
        onMainCardClick(card) {
          ...
          var rightBottomNode = this.bottomCards[this.rightSingleIdx];
          var rightCardView = rightBottomNode.getComponent(_crd && CardView === void 0 ? (_reportPossibleCrUseOfCardView({
            error: Error()
          }), CardView) : CardView);

          if (rightCardView && !rightCardView.cardData.isUsed) {
            var bottomValue = rightCardView.cardData.value;
        ...
        matchMainWithBottom(mainCardData, oldBottomNode, oldBottomView) {
          ...
          var mainNode = (_this$getCardViewById6 = this.getCardViewById(mainCardData.id)) == null ? void 0 : _this$getCardViewById6.node;
          ...
          var mainCardView = this.mainCards.find(cv => cv.cardData.id === mainCardData.id);

          if (mainCardView) {
            var index = this.mainCards.indexOf(mainCardView);
        ...
        onBottomSingleClick() {
          var rightCard = this.bottomCards[this.rightSingleIdx];
          var rightBtn = rightCard.getComponent(Button);
        ...
        updateBottomVisual() {
          ...
          var leftPileIds = this.bottomPileQueue.slice(1);

          if (leftPileIds.length >= 1) {
            var card1 = this.bottomCards[leftPileIds[0]];
            card1.active = true;
            card1.setPosition(this.leftPilePos1);
            var btn1 = card1.getComponent(Button);
          ...
          if (leftPileIds.length >= 2) {
            var card2 = this.bottomCards[leftPileIds[1]];
            card2.active = true;
            card2.setPosition(this.leftPilePos2);
            var btn2 = card2.getComponent(Button);
        ...
        getCardViewById(cardId) {
          var mainCard = this.mainCards.find(c => c.cardData.id === cardId);
          if (mainCard) return mainCard;

          for (var i = 0; i < this.bottomCards.length; i++) {
            var cardView = this.bottomCards[i].getComponent(_crd && CardView === void 0 ? (_reportPossibleCrUseOfCardView({
              error: Error()
            }), CardView) : CardView);
        ...
        getCardValueByIndex(index) {
          ...
          var cardView = (_this$bottomCards$ind = this.bottomCards[index]) == null ? void 0 : _this$bottomCards$ind.getComponent(_crd && CardView === void 0 ? (_reportPossibleCrUseOfCardView({
            error: Error()
          }), CardView) : CardView);
